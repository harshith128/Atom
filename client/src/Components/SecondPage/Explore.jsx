...
export const Explore = ({subCategory, category}) => {
    ...
    const getData = async() => {
        const res = await axios("http://localhost:2357/projects/subcategory?subcategory=Tabletop Games");
        console.log(active);
                    ...
                    <Stack spacing={2} direction="column">
                        {
                            subCategory.map((ele, i) => {
                                return (
                                    <button key={i} onClick={() => { handleSubCategory(ele) }} className={ active === ele ? "active" : "subcat" }>{ele}</button>
                                )
                            })
                        }
                    </Stack>
